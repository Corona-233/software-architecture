...
public class DeliveryOrderDao {
    ...
    public void updateDeliveryOrder(int id,String task_num,String project_name,String construction_site,String way,String plan_cubic_meter,String actual_cubic_meter,String production_date) throws SQLException {
        String sql ="update delivery_order set task_num=?,project_name=?,construction_site=?,way=?,plan_cubic_meter=?,actual_cubic_meter=?,production_date=? where id=?";
        try (Connection conn = getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setString(1, task_num);
            pstmt.setString(2, project_name);
